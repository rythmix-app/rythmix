...
export class UserService {
  ...
  getUsers(filters?: UserFilters): Observable<User[]> {
    return this.api
      .get<{ users: User[] }>(this.endpoint, filters)
      .pipe(map((response) => response.users));
  }

  getUserById(id: string): Observable<User> {
    return this.api
      .get<{ user: User }>(`${this.endpoint}/${id}`)
      .pipe(map((response) => response.user));
  }

  createUser(user: CreateUserDto): Observable<User> {
    return this.api
      .post<{ user: User }>(this.endpoint, user)
      .pipe(map((response) => response.user));
  }

  updateUser(id: string, user: UpdateUserDto): Observable<User> {
    return this.api
      .patch<{ user: User }>(`${this.endpoint}/${id}`, user)
      .pipe(map((response) => response.user));
  }
  ...
  restoreUser(id: string): Observable<User> {
    return this.api
      .post<{ user: User }>(`${this.endpoint}/${id}/restore`, {})
      .pipe(map((response) => response.user));
  }
  ...
  getTrashedUsers(): Observable<User[]> {
    return this.api
      .get<{ users: User[] }>(`${this.endpoint}/trashed`)
      .pipe(map((response) => response.users));
  }
}
